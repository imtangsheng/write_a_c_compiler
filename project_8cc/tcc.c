...
Ast *find_var(char *name){
    for (Ast *p = vars; p; p = pclose->next) {
...
Ast *read_indent_or_func(char c)  {
    char *name = read_indent(c);
    skip_space();
    char c2 = getc(stdin);
    if (c2 == '(')
...
Ast *read_prim(void){
    ...
    if (c == '"')
        return read_string();
    if (isalpha(c))
        return read_indent_or_func(c);
    else if (c == EOF)
        return NULL;
    error("Don't knoe how to hadle '%c'",c);
}
